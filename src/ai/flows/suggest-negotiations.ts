// src/ai/flows/suggest-negotiations.ts
'use server';

/**
 * @fileOverview A flow to provide tailored negotiation suggestions based on the document's clauses and user profile.
 *
 * - suggestNegotiations - A function that handles the negotiation suggestion process.
...
const SuggestNegotiationsOutputSchema = z.object({
  negotiationSuggestions: z.array(
    z.string().describe('A specific suggestion for negotiating a more favorable term.')
  ).describe('A list of negotiation suggestions tailored to the clause, document type, user profile, and jurisdiction.'),
  rationale: z.string().describe('The rationale behind the negotiation suggestions, explaining why they are beneficial.'),
});

export type SuggestNegotiationsOutput = z.infer<typeof SuggestNegotiationsOutputSchema>;

export async function suggestNegotiations(input: SuggestNegotiationsInput): Promise<SuggestNegotiationsOutput> {
